...
import 'package:mwntalassessment/assessment/inference_engine.dart';
import 'package:mwntalassessment/assessment/knowledge_base.dart';
import 'package:mwntalassessment/assessment/UI/result_page.dart';
...
class _ExpertSystemAppState extends State<ExpertSystemApp> {
  ...
  bool isKnowledgeBaseLoaded = false;
  ...
  Future<void> _loadKnowledgeBase() async {
    try {
      ...
      knowledgeBase = await KnowledgeBase.loadFromAssets('assets/knowledge_base.json');
      ...
      showDialog(
        ...
        builder: (BuildContext context) => AlertDialog(
          title: Text('Error'),
          content: Text('Failed to load knowledge base. Please try again later.'),
          actions: <Widget>[
            TextButton(
              onPressed: () {
  ...
  void _runInference(BuildContext context) {
    if (selectedFactList.length < 5) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please select at least five facts before submitting.')),
      );
      return;
    }
    ...
    Timer(Duration(seconds: 5), () {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => ResultPage(conclusion: conclusion)),
      );
    });
  }
  ...
  void _filterFacts() {
    setState(() {
      ...
      filteredFacts = knowledgeBase.facts
          .where((fact) => fact.toLowerCase().contains(query) && !selectedFactList.contains(fact))
          .toList();
    });
  }
      ...
      home: Scaffold(
        ...
        body: !isKnowledgeBaseLoaded
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: selectedFactList.length >= 5 && !isLoading ? () => _runInference(context) : null,
              child: const Text('Submit'),
              style: ElevatedButton.styleFrom(
                foregroundColor: selectedFactList.length >= 5 && !isLoading ? Colors.white : Colors.grey,
                backgroundColor: selectedFactList.length >= 5 && !isLoading ? Colors.blue : Colors.grey,
              ),
            ),
          ],
        ),
